...
//Global variable for sys_inb calls counting
unsigned int SIB_COUNTER = 0;
...
int keyboard_read_scancode_IH(){
  ...
  //Due to the potential delay of the KBC, the read operation is attempted several times with a small delay
	for(nretries = 0; nretries <= KBC_MAX_RETRIES; ++nretries) {
		...
		if(sys_inb_cnt(KBC_STATUS_REG, &statreg) != OK){
		...
		if(statreg & KBC_OBF) {
			...
			if((statreg & (KBC_PARITY | KBC_TIMEOUT | KBC_AUX )) == 0){
				if(sys_inb_cnt(KBC_OUT_BUF, &readOutbuf) != OK){
...
int keyboard_write_kbccommand(unsigned char kbccmd){
  ...
  for(nretries = 0; nretries < KBC_MAX_RETRIES; ++nretries){
    ...
    if(sys_inb_cnt(KBC_STATUS_REG, &status) != OK){
...
int keyboard_write_cmdbyte(unsigned char cmdbyte){
  ...
  //TODO: Ask teacher if we should try until the command is sent OR for a maximum number of times (also change in other 2 funcs)
  ...
  printf("keyboard_write_cmdbyte::The KBC command byte could be written after %d tries.\n", KBC_MAX_RETRIES);
  return -2;
}
